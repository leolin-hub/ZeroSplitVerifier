    ...
    def getMaximumEps(self, p, true_label, target_label, eps0 = 1, max_iter = 100, 
        ...
        with torch.no_grad():
            ...
            while (increase_u_eps.sum()>0):
                ...
                #Use new mask to avoid overlapping with the memory
                new_mask = torch.zeros_like(increase_u_eps)
                new_mask[increase_u_eps] = temp
                increase_u_eps = new_mask
            ...
            while(search.sum()>0):
                ...
                # Use new mask to avoid overlapping with the memory
                new_mask = torch.zeros_like(search)
                new_mask[search] = temp
                
                #l_eps[search] = eps[temp]
                # Update l_eps and u_eps
                l_eps[new_mask] = eps[temp]
                #increase active and true_lower>target_upper units in l_eps 
                
                #u_eps[search_copy-search] = eps[temp==0]
                u_eps[search & ~new_mask] = eps[~temp]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hidden-size', default = 2, type = int, metavar = 'HS',
                        help = 'hidden layer size (default: 64)') #改2原為64
    parser.add_argument('--time-step', default = 2, type = int, metavar = 'TS',
                        help = 'number of slices to cut the 28*28 image into, it should be a factor of 28 (default: 7)') #改2原為7
    parser.add_argument('--activation', default = 'relu', type = str, metavar = 'a',
                        help = 'nonlinearity used in the RNN, can be either tanh or relu (default: tanh)') #改relu原為tanh
    parser.add_argument('--work-dir', default = 'C:/Users/leolin9/POPQORN/models/mnist_classifier/rnn_2_2_relu/', type = str, metavar = 'WD',
    ...
    parser.add_argument('--N', default = 5, type = int,
                        help = 'number of samples to compute bounds for (default: 100)') # 改5原為100
