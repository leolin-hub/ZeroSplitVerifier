...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='LSTM POPQORN verifier — output bounds')
    ...
    parser.add_argument('--work-dir', default='../models/cifar10_lstm/', type=str, metavar='WD',
                        help='directory containing the pretrained model')
    parser.add_argument('--model-name', default='lstm', type=str, metavar='MN',
                        help='model filename inside work-dir (default: lstm)')
    ...
    model_path = os.path.join(args.work_dir, f'lstm_hidden_size_{hidden_size}', args.model_name)
    ...
    logger.info(f'\nComputing POPQORN output bounds (eps={args.eps}, p={p}) ...')
    yL, yU = get_last_layer_bound(
        lstm.W, lstm.b, lstm, X, args.eps, p, eps_idx=eps_idx
    )

    logger.info(f'\nyL (output lower bounds):\n{yL}')
    logger.info(f'\nyU (output upper bounds):\n{yU}')

    verified = True
    for i in range(yL.shape[0]):
        L = int(y[i].item())
        mask = torch.arange(output_size, device=device) != L
        if not (yL[i, L] > yU[i][mask]).all():
            verified = False
            break
    logger.info(f'\nPOPQORN verified: {verified}')

    logger.info('\nPer-sample analysis:')
    for i in range(X.shape[0]):
        L = int(y[i].item())
        mask = torch.arange(output_size, device=device) != L
        margin = float(yL[i, L] - yU[i][mask].max())
        logger.info(
            f'  Sample {i}: true={L}, '
            f'yL[true]={float(yL[i, L]):.4f}, '
            f'max_yU_others={float(yU[i][mask].max()):.4f}, '
            f'margin={margin:.4f}'
        )
