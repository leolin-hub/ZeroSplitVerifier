...
def get_d_UB(l,u,func,dfunc):
    ...
    keep_search = torch.ones_like(l, dtype=torch.bool)
    ...
    for i in range(max_iter):
        ...
        # Use new mask
        new_mask = torch.zeros_like(keep_search)
        new_mask[keep_search] = idx
        keep_search = new_mask
        ...
        idx = t>0
        active_mask = torch.zeros_like(keep_search)
        active_mask[keep_search] = idx
        ub[active_mask] = d[active_mask]
        d[active_mask] = (d[active_mask] + lb[active_mask]) / 2
      
        idx = t<0
        lb[active_mask] = d[active_mask]
        d[active_mask] = (d[active_mask] + ub[active_mask]) / 2
